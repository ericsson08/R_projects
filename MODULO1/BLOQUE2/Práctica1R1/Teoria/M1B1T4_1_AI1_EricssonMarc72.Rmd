...
ejercicio_2 <- ggplot(data = millas, mapping = aes(x =autopista, y = cilindrada, color = modelo, shape = traccion, size = anio)) + 
  geom_point() +
...
#Claramente podemos observar que cuanto más cilindrada, menos consumo.
...
ejercicio_5 <- function(x) {
  
  resultados <- list()
  
  resultados[["Last value"]] <- tail(x, n = 1)
  
  resultados[["Evens"]] <- x[c(TRUE,FALSE)]
  
  resultados[["All - first value"]] <- x[-1]
  
  resultados[["Only evens"]] <- x[x%%2 != 0]
...
ejercicio_10_1 <- subset(ejercicio_10, is.na(ejercicio_10$vol_por_cil))
print(nrow(ejercicio_10_1))

summary(ejercicio_10)

summary(ejercicio_10_1)
